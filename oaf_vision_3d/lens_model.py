...
def _distort_pixels(
    ...
    radial_coeff = (1 + distortion_coefficients.k1 * r2 + 
                   distortion_coefficients.k2 * r4 + 
                   distortion_coefficients.k3 * r6) / (
                   1 + distortion_coefficients.k4 * r2 + 
    ...
    x_tangential = distortion_coefficients.p1 * xy2 + distortion_coefficients.p2 * r2_2x2
    y_tangential = distortion_coefficients.p2 * xy2 + distortion_coefficients.p1 * r2_2y2
    ...
    if distortion_coefficients.tau_x != 0 or distortion_coefficients.tau_y != 0:
        tilt_matrix = np.array([
            ...
            [-np.sin(distortion_coefficients.tau_x) * np.sin(distortion_coefficients.tau_y),
             np.cos(distortion_coefficients.tau_y), 0.0],
            [np.sin(distortion_coefficients.tau_y),
             -np.sin(distortion_coefficients.tau_x) * np.cos(distortion_coefficients.tau_y),
             np.cos(distortion_coefficients.tau_x) * np.cos(distortion_coefficients.tau_y)]
        ], dtype=np.float32)
        
        # Apply tilt transformation
        distorted_homogeneous = np.stack([x_distorted, y_distorted, np.ones_like(x_distorted)], axis=-1)
